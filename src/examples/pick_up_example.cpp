...
public:
    WaitForObject(State* parent) : State(parent), event_object_found(this, "Object found"), obj_(new sbc15_msgs::Object)
    {
        subObject_ = GlobalState::getInstance().nh.subscribe<sbc15_msgs::Object>(
            "/leia/pickup_object", 1, std::bind(&WaitForObject::objectCb, this, std::placeholders::_1));
    }
...
int main(int argc, char* argv[])
{
    ...
    executor.run([&](State* current_state) {
        ...
        if (now > last_pub + state_pub_rate || last_state != current_state) {
