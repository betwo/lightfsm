...
void MoveitMotion::iteration()
{
    if (!started_ && retries_left_ > 0) {
        if ((takeGlobalStateGoal_ && GlobalState::getInstance().getCurrentArmGoal().valid) || !takeGlobalStateGoal_) {
            started_ = true;
            --retries_left_;
            //            client_.sendGoal(goal_,std::bind(&MoveitMotion::doneCb, this, std::placeholders::_1,
            //            std::placeholders::_2));
            GlobalState::getInstance().moveArmTo(
                goal_, std::bind(&MoveitMotion::doneCb, this, std::placeholders::_1, std::placeholders::_2));
        }
        if (withOffset_ && GlobalState::getInstance().getCurrentArmGoal().valid) {
            started_ = true;
            --retries_left_;
            //            clientOffset_.sendGoal(goalOffset_,std::bind(&MoveitMotion::doneCbOffset, this,
            //            std::placeholders::_1, std::placeholders::_2));
            GlobalState::getInstance().moveArmTo(goalOffset_, std::bind(&MoveitMotion::doneCbOffset, this,
                                                                        std::placeholders::_1, std::placeholders::_2));
        }
    }
}
