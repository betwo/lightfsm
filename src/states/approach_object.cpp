...
void ApproachObject::entryAction()
{
        ...
        if (o->type == type) {
            ...
            if (pos.z() > -0.35 && pos.z() < 0.35) {
                ...
                if ((angle < max_angle) || (M_PI - angle) < max_angle) {
                    ...
                    if (d < distance_ * 5) {
                        tf::Transform trafo_to_odom =
                            global.getTransform("/odom", "/arm_base_link", o->header.stamp, ros::Duration(0.5));
                        ...
                        if (error.length() < 0.5) {
                            ...
                            ROS_INFO_STREAM("drop object @ "
                                            << object_odom.getOrigin().x() << " / " << object_odom.getOrigin().y()
                                            << " with start pose " << start_pose_odom_.getOrigin().x() << " / "
                                            << start_pose_odom_.getOrigin().y() << " with offset " << error.x() << " / "
                                            << error.y());
                        }

                    } else {
                        ROS_INFO_STREAM("drop object  @ " << pose.getOrigin().x() << " / " << pose.getOrigin().y()
                                                          << " with start pose " << start_pose_odom_.getOrigin().x()
    ...
    tf::Transform trafo_to_odom =
        global.getTransform("/odom", current_object->header.frame_id, current_object->header.stamp, ros::Duration(2.0));
...
void ApproachObject::position(const sbc15_msgs::ObjectConstPtr& object_odom)
{
    ...
    ROS_INFO_STREAM("going to object @" << object_base_link.getOrigin().x() << "\t / "
                                        << object_base_link.getOrigin().y());
    ...
    switch (type) {
        case sbc15_msgs::Object::OBJECT_CUP: {
            ...
            tf::Pose error =
                tf::Pose(tf::createQuaternionFromYaw(std::atan2(desired_pos.y(), desired_pos.x())), desired_pos);
