                        ...
                        const path_msgs::NavigateToGoalResultConstPtr& result)
{
    if (result->status == path_msgs::NavigateToGoalResult::STATUS_SUCCESS) {
        ...
        if (retries_left_ > 0) {
            GlobalState::getInstance().moveTo(
                goal_, std::bind(&FollowPath::doneCb, this, std::placeholders::_1, std::placeholders::_2));
            --retries_left_;
        } else {
                        ...
                        std::function<void(const path_msgs::NavigateToGoalFeedbackConstPtr&)> cb)
{
    goal_ = target_msg;
    GlobalState::getInstance().moveTo(
        goal_, std::bind(&FollowPath::doneCb, this, std::placeholders::_1, std::placeholders::_2), cb);
    event.trigger();
}
