  ...
  , tfl_(ros::Duration(15.0))
{
    ...
    std::function<void(const std_msgs::Float32ConstPtr&)> set_velo = [&](const std_msgs::Float32ConstPtr& msg) {
    ...
    bool cup_collected =
        nh.param(std::string("object_") + std::to_string(sbc15_msgs::Object::OBJECT_CUP) + "_collected", false);
    if (cup_collected) {
        setObjectCollected(sbc15_msgs::Object::OBJECT_CUP);
    }
    bool battery_collected =
        nh.param(std::string("object_") + std::to_string(sbc15_msgs::Object::OBJECT_BATTERY) + "_collected", false);
...
void GlobalState::moveTo(
    const tf::Pose& target,
    std::function<void(const actionlib::SimpleClientGoalState&, const path_msgs::NavigateToGoalResultConstPtr&)> doneCb,
    int failure_mode, const std::string& planning_algorithm)
{
    moveTo(
        target, doneCb, [this](const path_msgs::NavigateToGoalFeedbackConstPtr& msg) { GlobalState::feedbackCb(msg); },
        failure_mode, planning_algorithm);
}

void GlobalState::moveTo(
    const tf::Pose& target,
    std::function<void(const actionlib::SimpleClientGoalState&, const path_msgs::NavigateToGoalResultConstPtr&)> doneCb,
    std::function<void(const path_msgs::NavigateToGoalFeedbackConstPtr&)> feedbackCb, int failure_mode,
...
void GlobalState::moveTo(
    const tf::Pose& target, double velocity,
    std::function<void(const actionlib::SimpleClientGoalState&, const path_msgs::NavigateToGoalResultConstPtr&)> doneCb,
    int failure_mode, const std::string& planning_algorithm)
{
    moveTo(
        target, velocity, doneCb,
        [this](const path_msgs::NavigateToGoalFeedbackConstPtr& msg) { GlobalState::feedbackCb(msg); }, failure_mode,
        planning_algorithm);
}
void GlobalState::moveTo(
    const tf::Pose& target, double velocity,
...
void GlobalState::moveTo(
    const geometry_msgs::PoseStamped& target_msg,
    std::function<void(const actionlib::SimpleClientGoalState&, const path_msgs::NavigateToGoalResultConstPtr&)> doneCb,
    int failure_mode, const std::string& planning_algorithm)
{
    moveTo(
...
void GlobalState::moveTo(
    const geometry_msgs::PoseStamped& target_msg, double velocity,
    std::function<void(const actionlib::SimpleClientGoalState&, const path_msgs::NavigateToGoalResultConstPtr&)> doneCb,
    int failure_mode, const std::string& planning_algorithm)
{
    moveTo(
        target_msg, velocity, doneCb,
        [this](const path_msgs::NavigateToGoalFeedbackConstPtr& msg) { GlobalState::feedbackCb(msg); }, failure_mode,
        planning_algorithm);
}
    ...
    std::function<void(const path_msgs::NavigateToGoalFeedbackConstPtr&)> feedbackCb)
{
    ...
    client_.sendGoal(goal, doneCb, std::bind(&GlobalState::activeCb, this), feedbackCb);
}

void GlobalState::moveArmTo(
...
void GlobalState::playRecordedTrajectory(
